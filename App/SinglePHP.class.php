...
function halt($str, $display=false){
    Log::warn($str);
    Log::fatal(' debug_backtrace:'.var_export(debug_backtrace(), true));
...
class SinglePHP {
    ...
    public function run(){
        ...
        if(strcmp(strtoupper($pathMod),'NORMAL') === 0 || !isset($_SERVER['PATH_INFO'])){
            ...
            $this->a = isset($_GET['a'])?$_GET['a']:'Download';
        }else{
    ...
    public static function autoload($class){
        if(substr($class,-10)=='Controller'){
            ...
            includeIfExist(C('APP_FULL_PATH') . '/Extend/' . str_replace('\\', '/', $class) . '.php');
        }
    }
}
...
class File{
    public static function save($name, $data) {
        $dataPath = C('APP_FULL_PATH') . '/Data/' . $name . '.php';
        $content = file_exists($dataPath) ? unserialize(myf_get_contents($dataPath)) : [];
        $key = $content ? count($content) : 0;
        if ($content) {
        ...
        $content[$key] = $data;
        myf_put_contents($dataPath, serialize($content));
    }
    ...
    public static function all($name) {
        ...
        if (empty($content))
            return [];
        return array_column($content, null, 'id');
    }

    public static function find($name, $key, $val) {
        $content = self::all($name, $key);
        return array_key_exists($val, $content)? $content[$val]:null;
    }

    public static function search($name, $key, $val=null, $page=1, $limit=10) {
        $content = self::all($name, $key);
        $result = [];
        foreach ($content as $data) {
            if (!array_key_exists($key, $data)) continue;
            if ($val) {
                if (strstr($data[$key], strval($val)) !== false) {
                    $result[] = $data;
                }
            } else {
                $result[] = $data;
        ...
        return [
            ...
            'limit'=>$limit,
            'count'=>count($result),
            'result'=>array_slice(array_reverse($result), ($page-1)*$limit, $limit)
        ];
    }

    public static function delete($filename, $key, $value)
    {
        $dataPath = C('APP_FULL_PATH') . '/Data/' . $filename . '.php';
        $content = file_exists($dataPath) ? unserialize(myf_get_contents($dataPath)) : [];
        if (empty($content))
            return false;
        $newContent = [];
        ...
        myf_put_contents($dataPath, serialize($newContent), LOCK_EX);
        return true;
    }
}
