...
var uploader = new plupload.Uploader({
    ...
    filters: {
        mime_types: [ //只允许上传图片和zip文件
            {title: "Image files", extensions: "apk,ipa"},
            {title: "Zip files", extensions: "apk,ipa"}
        ],
    ...
    init: {
        ...
        UploadProgress: function (up, file) {
            ...
            $('.step2 .progress-bar').css('width', file.percent + '%');
            $('#ossfile #percent-txt').text(file.percent + '%');
            $("input[name='size']").val(plupload.formatSize(file.size));
        },
        ...
        FileUploaded: function (up, file, info) {
            if (info.status == 200) {
                ...
                $.post('index.php?c=app&a=parse', {
                    ipa: get_uploaded_object_name(file.name),
                    'icon': app.icon
                }, function (r) {
                    ...
                    $("input[name='file']").val(host + "/" + get_uploaded_object_name(file.name));
